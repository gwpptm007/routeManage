...
int main(int argc, char *argv[]) {
    ...
    unsigned char uip[4] = {172, 16, 40, 117};  // 示例IP地址
    unsigned char gip[4] = {172, 16, 40, 253};  // 示例网关IP地址
	...
	if (strcmp(argv[1], "add") == 0) {
        printf("Adding route...\n");
        int result = wq_route_add_ip4(uip, gip, (unsigned char *)ifname, tabid);
        if (result == 0) {
        ...
        int result = wq_route_delete_ip4(uip, gip, (unsigned char *)ifname, tabid);
        if (result == 0) {
